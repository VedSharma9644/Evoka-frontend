...
function AllEvents({ language,expired }) {
      ...
      <div className="mb-8 flex flex-col sm:flex-row gap-4 items-center justify-between">
        <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
          <input
            ...
            className="p-2 border border-gray-200 dark:border-gray-600 rounded-xl bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-400 focus:border-transparent shadow-sm transition-all duration-200 w-full sm:w-64"
          />
          <input
            type="text"
            value={searchCity}
        ...
        <select
          ...
          className="p-2 border border-gray-200 dark:border-gray-600 rounded-xl bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-400 focus:border-transparent shadow-sm transition-all duration-200 w-full sm:w-48"
        >
      ...
      {filteredEvents.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {filteredEvents.map((event) => (
            <div
              key={event.id}
              className="bg-white dark:bg-gray-800 rounded-xl shadow-md w-full max-w-xs h-[350px] flex flex-col transform hover:shadow-lg transition-all duration-300"
            >
                ...
                <div className="relative">
                  ...
                  <div className="absolute top-2 left-2 flex gap-1">
                    {event.tags && event.tags.map((tag, index) => (
                      <span
                        key={index}
                        className="bg-black/60 dark:bg-white/20 text-white text-xs font-semibold px-2 py-1 rounded-full"
                      >
                        {tag.toUpperCase()}
                      </span>
                    ))}
                  </div>
                  <div className={`absolute top-2 right-2 text-white text-xs font-semibold px-2 py-1 rounded-full ${event.is_free ? 'bg-green-500 dark:bg-green-400' : 'bg-red-500 dark:bg-red-400'}`}>
                    <span>{event.is_free ? "FREE" : "PAID"}</span>
                  </div>
                </div>
              ) : (
                <div className="w-full h-40 bg-gray-200 dark:bg-gray-700 rounded-t-xl flex items-center justify-center">
                  <span className="text-gray-500 dark:text-gray-400">No Image</span>
                </div>
              )}
              <div className="flex-1 p-3">
                <h2 className="text-md font-bold text-gray-800 dark:text-gray-100 mb-1 line-clamp-1">
                  {event.title}
                </h2>
              ...
              <Link
                to={`/events/${event.id}`}
                className="m-3 flex items-center justify-center gap-1 py-1 px-3 bg-indigo-600 dark:bg-indigo-500 text-white rounded-lg hover:bg-indigo-700 dark:hover:bg-indigo-600 transition-all duration-200 text-sm"
              >
